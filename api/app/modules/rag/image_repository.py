...
class ImageRepository(Repository):
    ...
    async def get_by_chunk_id(self, chunk_id: int) -> list[Image]:
        stmt = select(Image).where(Image.chunk_id == chunk_id).order_by(Image.image_index)
        result = await self._session.exec(stmt)
        return list(result.all())

    async def get_by_chunk_ids(self, chunk_ids: list[int]) -> list[Image]:
        stmt = select(Image).where(Image.chunk_id.in_(chunk_ids)).order_by(Image.chunk_id, Image.image_index)
        result = await self._session.exec(stmt)
        return list(result.all())

    async def get_by_file_id(self, file_id: int) -> list[Image]:
        stmt = select(Image).where(Image.file_id == file_id).order_by(Image.image_index)
        result = await self._session.exec(stmt)
        return list(result.all())
